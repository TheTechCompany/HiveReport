...
export const MenuItem : React.FC<TreeItemContentProps> = forwardRef<HTMLDivElement, TreeItemContentProps>((props, ref) => {
    const {
      ...
      const { onEdit, onAdd, onSelect, selected: checked } = useContext(TreeViewContext);
    ...
    return (
        <Box 
            ...
            <Checkbox
                checked={checked?.indexOf(props.nodeId) > -1}
                onChange={(e) => onSelect(props.nodeId, e.target.checked)}
                />
            <Typography
