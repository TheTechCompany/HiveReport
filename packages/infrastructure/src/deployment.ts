...
export const Deployment = (provider: Provider, rootServer: string, dbUrl: Output<any>, dbPass: Output<any>) => {
    ...
    const appName = `hive-report-${suffix}`
                ...
                spec: {
                    containers: [{
                        ...
                        env: [
                            ...
                            { name: "DATABASE_URL", value: all([dbUrl, dbPass]).apply(([url, pass]) => `postgresql://postgres:${pass}@${url}.default.svc.cluster.local:5432/hivereport`) },
